...
pub fn init_control<'a>(
    control: std::sync::Arc<ControlState>,
) -> Result<MidiInputConnection<std::sync::Arc<ControlState>>, std::io::Error> {
    let midi_input = MidiInput::new("midic_test").map_err(|_| {
        std::io::Error::new(std::io::ErrorKind::Other, "could not open MIDI driver")
    })?;
    ...
    if let Some(i) = idx {
        ...
        return Err(std::io::Error::new(
            ...
            "could not locate the nanoKONTROL2 device",
        ));
    }
    ...
    let conn = midi_input
        ...
        .map_err(|_| {
            std::io::Error::new(std::io::ErrorKind::Other, "could not connect to MIDI port")
        })?;
    Ok(conn)
}
